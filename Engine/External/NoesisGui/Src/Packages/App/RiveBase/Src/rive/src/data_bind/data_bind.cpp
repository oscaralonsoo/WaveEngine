...
void DataBind::addDirt(ComponentDirt value, bool recurse)
{
    ...
    if (int(m_Dirt & ComponentDirt::Dependents) != 0 && m_ContextValue != nullptr)
    {
        m_ContextValue->invalidate();
    }
}
...
{
    auto flagsValue = static_cast<DataBindFlags>(flags());
    return int(flagsValue & (DataBindFlags::TwoWay | DataBindFlags::ToSource)) !=
           0;
}
...
{
    auto flagsValue = static_cast<DataBindFlags>(flags());
    return int(flagsValue & DataBindFlags::TwoWay) != 0 ||
           int(flagsValue & DataBindFlags::ToSource) == 0;
}
