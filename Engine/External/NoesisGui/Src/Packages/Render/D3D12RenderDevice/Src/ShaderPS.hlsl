...
Out main(in In i)
{
    ...
    #elif PAINT_LINEAR
        half4 paint = (half4)ramps.Sample(rampsSampler, i.uv0);
        half opacity_ = (half)opacity;

    #elif PAINT_RADIAL
        half dd = half(radialGrad1.x * i.uv0.x - radialGrad1.y * i.uv0.y);
        half u = half(radialGrad0.x * i.uv0.x + radialGrad0.y * i.uv0.y + radialGrad0.z *
            sqrt(i.uv0.x * i.uv0.x + i.uv0.y * i.uv0.y - dd * dd));
        half4 paint = (half4)ramps.Sample(rampsSampler, half2(u, radialGrad1.z));
        half opacity_ = (half)radialGrad0.w;

    #elif PAINT_PATTERN
        #if CUSTOM_PATTERN
            half4 paint = main_brush(i.uv0);
        #elif CLAMP_PATTERN
            float inside = all(i.uv0 == clamp(i.uv0, i.rect.xy, i.rect.zw));
            half4 paint = half4(inside * pattern.Sample(patternSampler, i.uv0));
            ...
            half4 paint = half4(inside * pattern.SampleGrad(patternSampler, uv, ddx(i.uv0), ddy(i.uv0)));
        #else
            half4 paint = (half4)pattern.Sample(patternSampler, i.uv0);
        #endif
        half opacity_ = (half)opacity;
    #endif
    ...
    /////////////////////////////////////////////////////
    ...
    #elif EFFECT_OPACITY
        o.color = (half4)image.Sample(imageSampler, i.uv1) * (opacity_ * paint.a);

    #elif EFFECT_SHADOW
        half2 uv = (half2)clamp(i.uv1 - shadowOffset, i.rect.xy, i.rect.zw);
        half alpha = (half)lerp(image.Sample(imageSampler, uv).a, shadow.Sample(shadowSampler, uv).a, blend);
        half4 img = (half4)image.Sample(imageSampler, clamp(i.uv1, i.rect.xy, i.rect.zw));
        o.color = (img + (1.0 - img.a) * ((half4)shadowColor * alpha)) * (opacity_ * paint.a);

    #elif EFFECT_BLUR
        o.color = half4(lerp(image.Sample(imageSampler, i.uv1), shadow.Sample(shadowSampler, i.uv1), blend)) * (opacity_ * paint.a);

    #elif EFFECT_SDF
        half4 color = (half4)glyphs.Sample(glyphsSampler, i.uv1);
        half distance = SDF_SCALE * (color.r - SDF_BIAS);

        #if 1
            half2 grad = (half2)ddx(i.st1.xy);
        #else
        ...
        o.color = (half4)lerp(image.Sample(imageSampler, i.uv1), pattern.Sample(patternSampler, i.uv0), i.color.a);
