static const char* Shader_140_frag = R"(
...
void main()
{
    ...
    /////////////////////////////////////////////////////
        ...
        vec4 paint = texture(ramps, vec2(u, cbuffer0_ps[6]));
        float opacity_ = cbuffer0_ps[3];

    #elif defined(PAINT_PATTERN)
        #if defined(CUSTOM_PATTERN)
            vec4 paint = main_brush(uv0);
        #elif defined(CLAMP_PATTERN)
            float inside = float(uv0 == clamp(uv0, rect.xy, rect.zw));
            vec4 paint = inside * texture(pattern, uv0);
        #elif defined(REPEAT_PATTERN) || defined(MIRRORU_PATTERN) || defined(MIRRORV_PATTERN) || defined(MIRROR_PATTERN)
            vec2 uv = (uv0 - tile.xy) / tile.zw;
            ...
            float inside = float(uv == clamp(uv, rect.xy, rect.zw));
            vec4 paint = inside * textureGrad(pattern, uv, dFdx(uv0), dFdy(uv0));
        #else
    ...
    /////////////////////////////////////////////////////
        ...
        vec4 shadowColor = vec4(cbuffer1_ps[0], cbuffer1_ps[1], cbuffer1_ps[2], cbuffer1_ps[3]);
        vec2 offset = vec2(cbuffer1_ps[4], -cbuffer1_ps[5]);
        vec2 uv = clamp(uv1 - offset, rect.xy, rect.zw);
        float alpha = mix(texture(image, uv).a, texture(shadow, uv).a, cbuffer1_ps[6]);
        vec4 img = texture(image, clamp(uv1, rect.xy, rect.zw));
