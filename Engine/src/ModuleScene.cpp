...
void ModuleScene::HandleFireworkInput() {
...
void ModuleScene::CreateFirework() {
    GameObject* rocket = CreateGameObject("Firework_Rocket");

    Transform* tr = static_cast<Transform*>(rocket->GetComponent(ComponentType::TRANSFORM));
    float randomX = ((rand() % 200) / 10.0f) - 10.0f;
    float randomZ = ((rand() % 200) / 10.0f) - 10.0f;
    tr->SetPosition(glm::vec3(randomX, 0.5f, randomZ));

    ComponentParticleSystem* particleComp = static_cast<ComponentParticleSystem*>(rocket->CreateComponent(ComponentType::PARTICLE));

    if (particleComp && particleComp->emitter)
    {
        particleComp->emitter->emissionRate = 60.0f;
        particleComp->emitter->maxParticles = 100;
        particleComp->emitter->additiveBlending = true;

        for (auto mod : particleComp->emitter->modules)
        {
            if (mod->type == ParticleModuleType::SPAWNER)
            {
                ModuleEmitterSpawn* spawn = static_cast<ModuleEmitterSpawn*>(mod);
                spawn->shape = EmitterShape::CONE;
                spawn->coneAngle = 5.0f;
                spawn->lifetimeMin = 0.2f;
                spawn->lifetimeMax = 0.4f;
                spawn->sizeStart = 0.2f;
                spawn->sizeEnd = 0.0f;
                spawn->colorStart = glm::vec4(1.0f, 0.9f, 0.6f, 1.0f);
                spawn->colorEnd = glm::vec4(1.0f, 0.2f, 0.0f, 0.0f);
            }
            if (mod->type == ParticleModuleType::MOVEMENT)
            {
                ModuleEmitterMovement* move = static_cast<ModuleEmitterMovement*>(mod);
                move->gravity = glm::vec3(0, -5.0f, 0);
            }
        }
    }

    glm::vec3 randomColor = glm::vec3((rand() % 100) / 100.0f,(rand() % 100) / 100.0f, (rand() % 100) / 100.0f);
    if (glm::length(randomColor) < 0.3f) randomColor += glm::vec3(0.5f);

    FireworkRocket fw;
    fw.go = rocket;
    fw.fuseTimer = 0.5f + ((rand() % 50) / 100.0f);
    fw.exploded = false;
    fw.color = randomColor;

    activeRockets.push_back(fw);

    LOG_CONSOLE("Firework launched");

}

void ModuleScene::CreateExplosion(glm::vec3 position, glm::vec3 color) {

    GameObject* explosion = CreateGameObject("Firework_Explosion");
    Transform* tr = static_cast<Transform*>(explosion->GetComponent(ComponentType::TRANSFORM));
    tr->SetPosition(position);

    ComponentParticleSystem* particleComp = static_cast<ComponentParticleSystem*>(explosion->CreateComponent(ComponentType::PARTICLE));

    if (particleComp && particleComp->emitter)
    {
        particleComp->emitter->emissionRate = 0.0f;
        particleComp->emitter->maxParticles = 500;
        particleComp->emitter->additiveBlending = true;
        ...
        for (auto mod : particleComp->emitter->modules)
        {
            if (mod->type == ParticleModuleType::SPAWNER)
            {
                ...
                spawn->shape = EmitterShape::SPHERE;
                spawn->emissionRadius = 0.5f;
                spawn->lifetimeMin = 0.8f;
                spawn->lifetimeMax = 1.5f;
                spawn->speedMin = 3.0f;
                spawn->speedMax = 7.0f;
                spawn->sizeStart = 0.5f;
                spawn->sizeEnd = 0.0f;
                spawn->colorStart = glm::vec4(color, 1.0f);
                spawn->colorEnd = glm::vec4(color, 0.0f);
            }
            if (mod->type == ParticleModuleType::MOVEMENT)
            {
                ModuleEmitterMovement* move = static_cast<ModuleEmitterMovement*>(mod);
                move->gravity = glm::vec3(0, -3.0f, 0);
            }
        }

        particleComp->emitter->Burst(150);
    }

    FireworkExplosion exp;
    exp.go = explosion;
    exp.lifeTime = 2.0f;
...
void ModuleScene::UpdateFireworksLogic() {

    float dt = Application::GetInstance().time->GetDeltaTime();

    auto itRocket = activeRockets.begin();

    while (itRocket != activeRockets.end())
    {
        FireworkRocket& fw = *itRocket;

        if (fw.go == nullptr) {
            itRocket = activeRockets.erase(itRocket);
            continue;
        }
        ...
        if (!fw.exploded)
        {
            Transform* tr = static_cast<Transform*>(fw.go->GetComponent(ComponentType::TRANSFORM));
            glm::vec3 newPos = tr->GetPosition() + glm::vec3(0, 8.0f * dt, 0);
            tr->SetPosition(newPos);

            ComponentParticleSystem* pComp = static_cast<ComponentParticleSystem*>(fw.go->GetComponent(ComponentType::PARTICLE));
            if (pComp && pComp->emitter) {
                for (auto mod : pComp->emitter->modules) {
                    if (mod->type == ParticleModuleType::SPAWNER) {
                        static_cast<ModuleEmitterSpawn*>(mod)->spawnPosition = newPos;
                    }
                }
            }

            if (fw.fuseTimer <= 0.0f)
            {
                CreateExplosion(tr->GetPosition(), fw.color);
                fw.go->MarkForDeletion();
                fw.exploded = true;
            }
            itRocket++;
        } else { itRocket = activeRockets.erase(itRocket); }
    }

    auto itExplosion = activeExplosions.begin();

    while (itExplosion != activeExplosions.end())
    {
        FireworkExplosion& exp = *itExplosion;

        if (exp.go == nullptr) {
            itExplosion = activeExplosions.erase(itExplosion);
            continue;
        }

        exp.lifeTime -= dt;

        if (exp.lifeTime <= 0.0f)
        {
            exp.go->MarkForDeletion();
